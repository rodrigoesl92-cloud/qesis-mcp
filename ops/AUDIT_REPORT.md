...
Generated 2026-08-26T00:13:11Z by `scripts/audit_ecosystem.py`.
...
## Verdict: NOT GREEN, 1 failing (5 informational)
...
| 9 | qesis-mcp: preflight, CI's own steps | **PASS** | 0 | gate: exit code is the contract |
| 10 | qesis-mcp: self-heal loop, dry run (the `heal` check) | **PASS** | 0 | gate: exit code is the contract |
| 11 | sovereign-infra: ledger mirror in sync | **PASS** | 0 | gate: exit code is the contract |
| 12 | sovereign-infra: ledger singleton | **PASS** | 0 | gate: exit code is the contract |
| 13 | sovereign-infra: ledger fixtures | **PASS** | 0 | gate: exit code is the contract |
| 14 | sovereign-infra: ecosystem bootstrap | **PASS** | 0 | gate: exit code is the contract |
| 15 | sovereign-infra: ecosystem fixtures | **PASS** | 0 | gate: exit code is the contract |
| 16 | sovereign-infra: RDL delta gate | **PASS** | 0 | gate: exit code is the contract |
| 17 | sovereign-infra: workflow contract | **PASS** | 0 | gate: exit code is the contract |
| 18 | sovereign-infra: self-heal fixtures | **PASS** | 0 | gate: exit code is the contract |
| 19 | sovereign-infra: preflight, CI's own steps | **PASS** | 0 | gate: exit code is the contract |
| 20 | sovereign-infra: self-heal loop, dry run (the `heal` check) | **PASS** | 0 | gate: exit code is the contract |
| 21 | compliance chain | **PASS** | 0 | predicate: every prev_hash equals the previous entry_hash (0 breaks) |
| 22 | qesis-mcp: open pull requests | **PASS** | 0 | measurement: 0 open pull requests |
| 23 | qesis-mcp: main | **INFO** | 0 | measurement: recorded for the deployment comparison |
| 24 | qesis-mcp: required checks on main | **PASS** | 0 | measurement: all 1 required check(s) on main are success; other check runs (e.g. Cloud Build) are informationa |
| 25 | qesis-mcp: open issues | **INFO** | 0 | measurement: 0 open issue(s), recorded |
| 26 | sovereign-infra: open pull requests | **INFO** | 0 | measurement: 1 open, none DIRTY. Open is unlanded work, not a defect; the lander merges or closes it |
| 27 | sovereign-infra: main | **INFO** | 0 | measurement: recorded for the deployment comparison |
| 28 | sovereign-infra: required checks on main | **FAIL** | 0 | measurement: ruleset unreadable, so every ecosystem-owned check on main must be success; not: heal=failure, qe |
| 29 | sovereign-infra: open issues | **INFO** | 0 | measurement: 0 open issue(s), recorded |
| 30 | live /health | **PASS** | 0 | measurement: status ok, chain VERIFIED with 0 breaks, attestation agrees, and deployment_commit equals main |
| 31 | landing page HTTP | **PASS** | 0 | measurement: HTTP 200, predicate: 200 |
...
LEDGER SYNC: 2 reachable cop(y/ies), repository qesis-mcp
  canonical    C:\Users\Lenovo\qesis-mcp\ops\LESSONS_LEDGER.md  entries 157 max L-174 sha256 c9a259e7c636
  canonical    C:\Users\Lenovo\OneDrive\sovereign-infra\ops\LESSONS_LEDGER.md  entries 157 max L-174 sha256 c9a259e7c636
...
ledger: C:\Users\Lenovo\qesis-mcp\ops\LESSONS_LEDGER.md  (repository: qesis-mcp)
  entries 157, unique 157, max L-174, sha256 c9a259e7c6369ce6
  R1  no duplicate id
...
RDL DELTA GATE PASSED: 5 accepted, 0 regressions. The ladder introduces no new escalation in this change set.
```
...
### 9. qesis-mcp: preflight, CI's own steps  (PASS, exit 0)
...
### 10. qesis-mcp: self-heal loop, dry run (the `heal` check)  (PASS, exit 0)
...
### 11. sovereign-infra: ledger mirror in sync  (PASS, exit 0)
...
### 12. sovereign-infra: ledger singleton  (PASS, exit 0)
...
### 13. sovereign-infra: ledger fixtures  (PASS, exit 0)
...
### 14. sovereign-infra: ecosystem bootstrap  (PASS, exit 0)
...
### 15. sovereign-infra: ecosystem fixtures  (PASS, exit 0)
...
### 16. sovereign-infra: RDL delta gate  (PASS, exit 0)
...
### 17. sovereign-infra: workflow contract  (PASS, exit 0)
...
### 18. sovereign-infra: self-heal fixtures  (PASS, exit 0)
...
### 19. sovereign-infra: preflight, CI's own steps  (PASS, exit 0)
...
### 20. sovereign-infra: self-heal loop, dry run (the `heal` check)  (PASS, exit 0)
...
### 21. compliance chain  (PASS, exit 0)
...
### 22. qesis-mcp: open pull requests  (PASS, exit 0)
...
```
main 24a6a80970b3  fix(gates): the singleton gate compares the sibling, the bootstrap ign
```

### 24. qesis-mcp: required checks on main  (PASS, exit 0)
...
### 26. sovereign-infra: open pull requests  (INFO, exit 0)
...
```
PR 41 head fix/land-20260824-consolidated MERGEABLE UNSTABLE
```

### 27. sovereign-infra: main  (INFO, exit 0)
...
```
main 5c8d11924b63  feat(RDL): pair the ladder executor, the singleton gate and the cloud 
```

### 28. sovereign-infra: required checks on main  (FAIL, exit 0)

`gh api repos/rodrigoesl92-cloud/sovereign-infra/commits/main/check-runs?per_page=100`  in `C:\Users\Lenovo\qesis-mcp`

Basis: measurement: ruleset unreadable, so every ecosystem-owned check on main must be success; not: heal=failure, qesis-integrity=failure
...
informational guard: success
informational heal: failure
informational qesis-integrity: failure
informational report: failure
informational verify: success
```

### 29. sovereign-infra: open issues  (INFO, exit 0)
...
```
status ok  vintage v9.0 (2026-08-13)  chain VERIFIED 752 entries 0 breaks
deployment_commit 24a6a80970b3  main 24a6a80970b3
tools 8  database connected
```

### 31. landing page HTTP  (PASS, exit 0)
...
- **sovereign-infra: required checks on main**: measurement: ruleset unreadable, so every ecosystem-owned check on main must be success; not: heal=failure, qesis-integrity=failure — `informational verify: success`
