...
| **(b)** | "100% of GitHub Actions pinned to full-length commit SHAs per DevSecOps 2026 standards" | Was **zero of ten**. Now **8 of 10**, remediated 2026-08-13. `actions/checkout@v4` pinned to `34e11487...` and `actions/setup-python@v5` to `a26af69b...`, both resolved from the GitHub ref API. The two `actions/github-script@v7` lines in `production-probe.yml` remain **UNPINNED**: the API did not return a ref and **a supply-chain SHA is never guessed**. They carry an inline resolve command. Control wired as `scripts/verify_action_pinning.py`, which FAILS strict and PASSES only under an explicit `--allow-unresolved actions/github-script` declaration, so the exception is visible rather than silent | `python scripts/verify_action_pinning.py` |
